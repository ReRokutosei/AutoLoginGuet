...
do_login() {
    local full_username="${USERNAME}${ISP}"
    local params="callback=dr1003&DDDDD=${full_username}&upass=${PASSWORD}&0MKKey=123456"
    
    local response=$(curl -s -X POST "${LOGIN_IP}/drcom/login" -d "${params}" 2>/dev/null)
    ...
    case $left_flow in
        ...
        *)
            if (( $(echo "$left_flow < 1024.0" | bc -l) )); then
                ...
                printf "剩余流量 %.2f GB" "$(echo "$left_flow / 1024.0" | bc -l)"
            fi
            ;;
    esac
}
...
main() {
    ...
    case $login_result in
        "SUCCESS")
            ...
            ;;
        "CONFIG_ERROR")
            login_message="登录失败！请检查配置信息"
            ;;
        "UNKNOWN_ERROR")
            login_message="登录异常！请检查代理或硬件"
            ;;
        "NETWORK_ERROR")
            login_message="登录失败！网络请求错误"
            ;;
        *)
            login_message="登录状态未知"
            ;;
    esac
    
    if [[ $login_result == "SUCCESS" ]] && [[ $flow_result != "" ]] && [[ $? -eq 0 ]]; then
        message="${login_message}${flow_result}"
    else
