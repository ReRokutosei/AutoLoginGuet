...
pub fn process_gui_events(
    ...
    for msg in receiver.try_iter() {
        match msg {
            ...
            GuiEventHandlerMessage::LoginAttempted { success: _, message: msg, elapsed_time: _ } => {
                message.write().clone_from(&msg);
            }
            GuiEventHandlerMessage::ConfigSaved { success: _, message: msg } => {
                message.write().clone_from(&msg);
            }
            GuiEventHandlerMessage::AutoStartSet { enabled: _, success: _, message: msg } => {
                message.write().clone_from(&msg);
            }
            GuiEventHandlerMessage::LogRecorded { level, message: msg } => {
                let message_center = MessageCenter::default();
                let _ = message_center.log_message(&level, &msg, None);
            }
        }
    }
}
    ...
    fn handle_event(&self, event: AppEvent) {
        match event {
            AppEvent::NetworkStatusChecked { status: _, message } => {
                let _ = self.sender.send(GuiEventHandlerMessage::NetworkStatusChecked {
                    message: message.to_string(),
                });
            }
            AppEvent::LoginAttempted { success, message, elapsed_time } => {
            ...
            AppEvent::AutoStartSet { enabled, success, message } => {
