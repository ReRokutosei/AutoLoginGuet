...
pub fn map_reqwest_error(e: reqwest::Error) -> NetworkError {
    ...
    } else if error_str.contains("tls") || error_str.contains("TLS") || error_str.contains("certificate") {
...
pub fn generate_user_friendly_message(error: &AppError) -> String {
    match error {
        ...
        AppError::PasswordDecryptionError { user_msg, .. } => {
            user_msg.clone()
        },
...
pub fn generate_login_error_message(error: &AppError) -> String {
    match error {
        AppError::NetworkError { source: NetworkError::ConnectionTimeout(_) } => "连接超时".to_string(),
        AppError::NetworkError { source: NetworkError::DnsError(_) } => "DNS解析失败".to_string(),
        AppError::NetworkError { source: NetworkError::TlsError(_) } => "TLS连接错误".to_string(),
...
pub fn generate_network_status_error_message(error: &AppError) -> String {
    match error {
        AppError::NetworkError { source: NetworkError::ConnectionTimeout(_) } => {
            "网络状态检查超时".to_string()
        },
        AppError::NetworkError { source: NetworkError::DnsError(_) } => {
            "DNS解析失败，无法解析校园网地址".to_string()
        },
        AppError::NetworkError { source: NetworkError::TlsError(_) } => {
            "TLS连接错误".to_string()
        },
        _ => {
